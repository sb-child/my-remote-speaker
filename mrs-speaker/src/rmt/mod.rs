...
use crate::rmt::{endpoint::RpcEp, keypair::KeypairService, sample_store::SampleStoreService};
...
pub async fn bind_endpoint(
    ...
    let ep = iroh::Endpoint::builder(presets::N0)
        ...
        .bind()
        .await?;
    let ctx = State::new(sample_store.clone(), ep.clone(), task_manager.clone());
...
impl State {
    pub fn new(sample_store: Tree, endpoint: Endpoint, task_manager: TaskManager) -> Self {
        Self {
            sample_store,
            endpoint,
            task_manager,
